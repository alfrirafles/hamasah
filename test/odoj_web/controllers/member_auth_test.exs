defmodule OdojWeb.MemberAuthTest do
  ...
  @remember_me_cookie "_odoj_web_member_remember_me"

  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, OdojWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_member/3" do
    test "stores the member token in the session", %{conn: conn, member: member} do
      ...
      assert Users.get_member_by_session_token(token)
    end
  ...
  describe "logout_member/1" do
    test "erases session and cookies", %{conn: conn, member: member} do
      member_token = Users.generate_member_session_token(member)
      ...
      assert redirected_to(conn) == "/"
      refute Users.get_member_by_session_token(member_token)
    end

    test "broadcasts to the given live_socket_id", %{conn: conn} do
      live_socket_id = "members_sessions:abcdef-token"
      OdojWeb.Endpoint.subscribe(live_socket_id)
      ...
      assert_receive %Phoenix.Socket.Broadcast{
        event: "disconnect",
        topic: "members_sessions:abcdef-token"
      }
    end
