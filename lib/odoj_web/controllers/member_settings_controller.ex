defmodule OdojWeb.MemberSettingsController do
  ...
  def update(conn, %{"action" => "update_email"} = params) do
    ...
    case Users.apply_member_email(member, password, member_params) do
      {:ok, applied_member} ->
  ...
  def update(conn, %{"action" => "update_password"} = params) do
    ...
    case Users.update_member_password(member, password, member_params) do
      {:ok, member} ->
        conn
  ...
  def confirm_email(conn, %{"token" => token}) do
    case Users.update_member_email(conn.assigns.current_member, token) do
      :ok ->
        conn
  ...
  defp assign_email_and_password_changesets(conn, _opts) do
    ...
    conn
    |> assign(:email_changeset, Users.change_member_email(member))
    |> assign(:password_changeset, Users.change_member_password(member))
  end
end
