defmodule OdojWeb.MemberAuth do
  import Plug.Conn
  import Phoenix.Controller

  alias Odoj.Users
  alias OdojWeb.Router.Helpers, as: Routes
  ...
  def log_out_member(conn) do
    ...
    member_token && Users.delete_session_token(member_token)

    if live_socket_id = get_session(conn, :live_socket_id) do
      OdojWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def fetch_current_member(conn, _opts) do
    ...
    member = member_token && Users.get_member_by_session_token(member_token)
  ...
  def require_authenticated_member(conn, _opts) do
    if conn.assigns[:current_member] do
      ...
      conn
      |> put_flash(:error, "Login untuk mengakses halaman")
  ...
  defp signed_in_path(_conn), do: "/catat"
end
