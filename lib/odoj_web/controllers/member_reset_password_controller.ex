defmodule OdojWeb.MemberResetPasswordController do
  ...
  def create(conn, %{"member" => %{"email" => email}}) do
    if member = Users.get_member_by_email(email) do
      Users.deliver_member_reset_password_instructions(
        member,
  ...
  def edit(conn, _params) do
    render(conn, "edit.html", changeset: Users.change_member_password(conn.assigns.member))
  end
  ...
  def update(conn, %{"member" => member_params}) do
    case Users.reset_member_password(conn.assigns.member, member_params) do
      {:ok, _} ->
        conn
  ...
  defp get_member_by_reset_password_token(conn, _opts) do
    ...
    if member = Users.get_member_by_reset_password_token(token) do
